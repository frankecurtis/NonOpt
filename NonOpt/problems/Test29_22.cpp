                               ...
                               double* x)
{
  ...
  // Set initial point
  for (int i = 1; i <= n; i++) {
    x[i-1] = i/(double)n*(i/(double)n-1.0);
  }
                                    ...
                                    double& f)
{
	...
	std::vector<double> term(n,0.0);
	  for (int i = 2; i < n-2; i++) {
		  term[i-1]=2*x[i-1]+0.5/pow(n+1,2)*pow((x[i-1]+i/(n+1.0)+1.0),3)-x[i-2]-x[i];
	  }
                                   ...
                                   double* g)
{
  ...
  if(max_ind==0){
	  if(term[max_ind]>=0){
		  g[max_ind]=2+1.5/pow(n+1,2)*pow(x[max_ind]+(max_ind+1)/(n+1.0)+1,2 );
		  g[max_ind+1]=-1.0;
	  }
	  else{
		  g[max_ind]=-2-1.5/pow(n+1,2)*pow(x[max_ind]+(max_ind+1)/(n+1.0)+1,2 );
