...
//                 f(x) = sum_{i=1..n-1}
//                          (-x_i + 2*(x_i^2 + x_{i+1}^2 - 1) + 1.75*|x_i^2 + x_{i+1}^2 - 1|)
//               with initial point
//                 x_i = -1.0 for all i = 1..n
// Notes       : THIS PROBLEM IS NONCONVEX
//               Optimal value: varies (for n=50, approx. -35.0)

#ifndef __CHAINEDMIFFLIN_2_HPP__
#define __CHAINEDMIFFLIN_2_HPP__
...
class ChainedMifflin_2 : public Problem
{
 ...
 private:
   ...
   * Copy constructor
   */
  ChainedMifflin_2(const ChainedMifflin_2&);
  /**
